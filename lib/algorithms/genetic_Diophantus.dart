...
String geneticAlgorithm(
  ...
  final resultIteration1 =
      geneticAlgorithmCalc(inputEquation, numberPopulations, maxIterations);
...
String geneticAlgorithmCalc(
  ...
  final inputCoefficients = List.generate(
      inputEquation.length, (index) => int.parse(inputEquation[index]));
  ...
  var currentPopulation = generateStartPopulation(
      numberPopulations, inputCoefficients.length, maxGeneValue);
  ...
  while (iterations == 0 || iterations > 0) {
    ...
    for (var i = 0; i < numberPopulations / 2; i++) {
      ...
      final mutatedGenes =
          mixedGenes.map((gene) => calcMutation(gene, maxGeneValue));
      currentPopulation..addAll(mutatedGenes);
    }
    if (maxIterations != 0) iterations--;
  }
}

int generateRandomValue(int max) => Random().nextInt(max);

List<List<int>> generateStartPopulation(
        int numberPopulations, int varNumber, int yMax) =>
    List.generate(
        numberPopulations,
        (index) =>
            List.generate(varNumber, (index) => generateRandomValue(yMax)));

int calcFitness(List<int> inputCoefficients, List<int> chromosome, int yValue) {
  var sum = 0;
  chromosome
      .asMap()
      .forEach((index, gene) => sum += gene * inputCoefficients[index]);
  return (yValue - sum).abs();
}

double calcInvertedSumDeltas(List<int> deltas) => deltas.fold<double>(
    0, (previousValue, currentValue) => previousValue + 1 / currentValue);
List<double> calcProbability(List<int> deltas) => deltas
    .map<double>((delta) => 1 / delta / calcInvertedSumDeltas(deltas))
    .toList();

List<List<int>> calcRoulette(List<Map<String, dynamic>> elements,
        {numWins = 2}) =>
...
List<int> selectRandom(List<Map<String, dynamic>> elements) {
  ...
  elements.forEach((element) => (randomValue -= element['probability']) < 0
      ? result.add(element['chromosome'])
      : null);
  return result[0];
}
...
List<int> calcMutation(List<int> chromosome, int maxGene,
    {double thresholdProbability = 0.1}) {
  ...
  if (thresholdProbability >= random) {
    return chromosome
        .asMap()
        .map((index, gene) => MapEntry(index, i == index ? value : gene))
        .values
        .toList();
  }
  return chromosome;
